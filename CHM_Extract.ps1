...
Get-ChildItem -Path $SourceDir -Filter *.chm | ForEach-Object {
    ...
    Get-ChildItem -Path $outputDir -Recurse | ForEach-Object {
        # Ensure the file actually exists, is not a directory, and is not empty
        if ($_.PSIsContainer -eq $false -and $_.FullName -ne $null -and $_.FullName -ne "" -and (Get-Item $_.FullName).Length -gt 0) {
            ...
            # Log empty or invalid files
            Add-Content -Path $ErrorLogFile -Value "Skipped empty or invalid file: $($_.FullName)"
            Write-Host "Skipped empty or invalid file:" $($_.FullName)
        }
    }
}
